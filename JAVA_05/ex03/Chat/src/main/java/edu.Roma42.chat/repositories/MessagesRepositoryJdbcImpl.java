...
public class	MessagesRepositoryJdbcImpl implements MessagesRepository {
	...
	public void	update(Message message) throws NotSavedSubEntityException {
		...
		try {
			stm = this.con.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_UPDATABLE);
			rs = stm.executeQuery("SELECT * FROM chat.Message WHERE id=" + msgID);
			if (!rs.next()) {
				throw new	NotSavedSubEntityException("Message id not found in db");
			}
			rs = stm.executeQuery("SELECT * FROM chat.User WHERE id=" + authorID);
			if (!rs.next()) {
				throw new	NotSavedSubEntityException("User id not found in db");
			...
			stm.execute("UPDATE chat.Message SET author = " + authorID + ", room = " + roomID + ", text = "
					+ message.getText() + ", time = " + message.getTime() + " WHERE id = " + msgID);
		} catch (SQLException e) {
			System.err.println(e);
		}
	}
